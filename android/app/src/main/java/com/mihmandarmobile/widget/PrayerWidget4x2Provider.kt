...
class PrayerWidget4x2Provider : AppWidgetProvider() {

    companion object {
        const val ACTION_WIDGET_CLICK = "com.mihmandarmobile.widget.4x2.WIDGET_CLICK"
        const val ACTION_REFRESH = "com.mihmandarmobile.widget.4x2.REFRESH"
    }
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (intent.action) {
            ...
            ACTION_REFRESH -> {
                ...
                val componentName = ComponentName(context, PrayerWidget4x2Provider::class.java)
    ...
    private fun updateWidget(context: Context, manager: AppWidgetManager, appWidgetId: Int) {
        val views = RemoteViews(context.packageName, com.mihmandarmobile.R.layout.prayer_widget_4x2)
        ...
        views.setTextViewText(com.mihmandarmobile.R.id.location_text, "Mihmandar")
    ...
    private fun setupClickIntents(context: Context, views: RemoteViews, appWidgetId: Int) {
        val clickIntent = Intent(context, PrayerWidget4x2Provider::class.java).apply {
        ...
        val refreshIntent = Intent(context, PrayerWidget4x2Provider::class.java).apply {
    ...
    private fun applyTheme(views: RemoteViews, themeData: String?) {
        themeData?.let {
            try {
                ...
                views.setTextColor(com.mihmandarmobile.R.id.location_text, textColor)
            } catch (e: Exception) {
                // Use default colors
            }
        }
        ...
        for ((name, time) in prayerTimes) {
            ...
            if (prayerTotalSeconds > currentTotalSeconds) {
                ...
                val remainingText = when {
                    hours > 0 -> "${hours} saat ${minutes} dakika ${seconds} saniye kaldı"
                    minutes > 0 -> "${minutes} dakika ${seconds} saniye kaldı"
                    else -> "${seconds} saniye kaldı"
                }
    ...
    private fun updateWidgetWithPrayerData(
        ...
        // Update hijri date
        val hijriDate = SimpleDateFormat("dd MMMM yyyy", Locale("tr")).format(Date())
        views.setTextViewText(com.mihmandarmobile.R.id.hijri_date, hijriDate)
